<template lang="pug">
    .modal.-is-active
        .modal__content
            button.modal__close(@click="closeModal") X
            img(:src="icon" alt="Seu cadastro foi solicitado com sucesso!")

            h1 Seu cadastro foi solicitado com sucesso! 

            router-link.modal__btn(to="/")
                svg(xmlns="http://www.w3.org/2000/svg" viewBox="0 0 428 266.66666")
                    g(transform="matrix(1.3333333,0,0,-1.3333333,0,266.66667)")
                        g(transform="scale(0.1)")
                            path(
                                fill="currentColor" fill-opacity="1" stroke="none"
                                d="M 3061.52,1141.24 H 472.93 l 617.66,617.73 c 54.14,54.06 54.14,141.8 0,195.86 -54.11,54.1 -141.84,54.1 -195.86,0 L 40.5898,1100.65 c -54.1093,-54.06 -54.1093,-141.798 0,-195.861 L 894.73,50.5703 C 921.762,23.5 957.23,9.98828 992.66,9.98828 c 35.43,0 70.89,13.51172 97.93,40.58202 54.14,54.1017 54.14,141.8007 0,195.8987 L 472.93,864.211 h 2588.59 c 76.49,0 138.52,62.027 138.52,138.509 0,76.49 -62.03,138.52 -138.52,138.52 z"
                            ) 
          


                span Voltar para home

</template>
<script>
export default {
    name: 'ModalSuccess',
    data() {
        return {
            icon: require('@/assets/img/icon-modal.png'),
        }
    },
    methods: {
        closeModal(){
            this.$emit('close')          
        }
    }
}
</script>